...
with st.sidebar:
    choose = option_menu("App Gallery", ["About","DataSet","Data Analysis","Data Visualization","Prediction","About Internship"],
        ...
        "nav-link": {"font-size": "16px", "text-align": "left", "margin":"0px", "--hover-color": "#fed8b1" , "color": "#000000"},
...
def prediction():
    ...
    if st.button('Predict House Price'):
        ...
        # assume predicted_price is a list or array with the predicted price in USD
        predicted_price_usd = predicted_price[0]

        # current exchange rate (1 USD = x INR)
        exchange_rate = 74.83  # update this value as needed
        ...
        # display the result
        st.header(f'Predicted House Price: ₹{predicted_price_inr:,.2f}')
        #st.header(f'Predicted House Price: ₹{predicted_price[0]:,.2f}')

def internship():

    st.title('Student and Comapny Details')

    # Student Details
    st.header('Student Details')
    st.write('Student Name: Shah Heer')
    st.write('Enrollment No.: 210570107041')
    st.write('Program: Computer Science')
    st.write('University: Marwadi University(GTU)')
    st.write('Contact Email: hir.shah18367@marwadieducation.edu.in')

    # Company Details
    st.header('Company Details')
    st.write('Company Name:  INFOLABZ IT SERVICES PVT. LTD.')
    st.write('Location: 405 Vraj Avenue,Above SAM S Pizza,Nr Commerce Six Road, Navrangpura, Ahmedabad 380009.')
    st.write('Industry: Information Technology')
    st.write('Website: https://infolabz.in/')
    st.write('Contact Email: contact@infolabz.in')
